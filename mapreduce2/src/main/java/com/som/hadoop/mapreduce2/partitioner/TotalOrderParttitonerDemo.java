...
public class TotalOrderParttitonerDemo extends Configured implements Tool {
  ...
  public static void main(String[] args) {
    ...
    try {
      int exitSttus = ToolRunner.run(new TotalOrderParttitonerDemo(), args);
  ...
  public int run(String[] args) throws Exception {
    ...
    job.setInputFormatClass(KeyValueTextInputFormat.class);// default key value
                                                           // separator is '\t'
    ...
    // inputformat key type and sampler key type must be same(mapper output
    // key,value type),
    // here keyvaluetextinputformat has key type Text and so keeping Text as
    // mapper output key class, or vice versa
    InputSampler.Sampler<Text, IntWritable> sampler =
        new InputSampler.RandomSampler<Text, IntWritable>(10.0, 1000000, 100);
    InputSampler.writePartitionFile(job, sampler);
